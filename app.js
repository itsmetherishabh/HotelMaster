...
app.post(
  ...
  [
    ...
    check("password", "Password must be of at least 8 characters ")
      .trim()
      .isLength({ min: 8 }),
            ...
            bcrypt.hash(user1.password, salt, (err, hash) => {
              ...
              user1
                ...
                .then((userl) => {
                  req.flash(
                    "success_msg",
                    "You are now registered, you can log in from here."
                  );
                  res.redirect("/login");
                })
                .catch((err) => console.log(err));
...
app.post("/book", ensureAuthenticated, async function (req, res) {
  ...
  for (var i = 0; i < l - 1; i++) {
    await Booking.find(
      { roomId: details[0]._id, roomtype: roomtype, checkIn: dateList[i] },
      function (err, rooms) {
